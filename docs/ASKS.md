...
Every request the user makes + every suggestion Claude proposes, with status.
...
## ⏳ Waiting on the user (nothing is blocked on Claude)
...
| J11 | Validate URLs before placing; keep a logger (right/wrong); backtest all + future; search online for the right URL | ✅ `src/link_check.py`: validate→classify (exact/weak/dead)→`data/apply_audit.csv`; runs in scan + app refresh; ✓/🔍 badge in queue; backtest + web-search resolver; **990 exact / 332 weak / 73 closed** (from 734/596) |
